...
当变更涉及 `gesture-anchor-frame`、`spatial-template-model` 或 `spatial-template-renderer` 时，必须执行本节。

1. 在有真实摄像头的设备上启动摄像头。
2. 只展示一个物理手。
3. 预期：
   - 顶部状态显示 `1 hand`，不能显示 `2 hands`；
   - 闭合拓扑版本渲染紧凑的 `A-B-C-D-E-A` 手部面；
   - 效果不是三角形、不是虚拟双手长条，也不是大面积扁平屏幕空间光片。
4. 保持一只手可见，并向左/右/上/下移动。
5. 预期：
   - 模板与手部可见方向一致移动；
   - 不出现镜像方向回归。
6. 展示两只物理手，并保持清晰间隔。
7. 预期：
   - 顶部状态显示 `2 hands`；
   - 模板变成更大的折叠多面三维模型；
   - 运动过程中至少能区分实时画面面、次级模板面、背面/强调侧面和边缘面。
8. 旋转或倾斜手部姿态。
9. 预期：
   - 面的可见性随透视变化；
   - 能看到侧面厚度或折叠边缘；
   - 效果不会退化成单个扁平透明四边形。
10. 将一只或两只手向摄像头前后远近移动。
11. 预期：
   - 可见透视/深度变化；
   - 物体不是只做二维平移。
12. 录制 20-30 秒，覆盖单手、双手、旋转、前后远近移动。
13. 将录屏与 `参考视频.mp4` 的抽帧拼图对比，确认当前结果朝多面三维模板收敛，而不是只像染色蒙版。

针对 `4159dbe` 之后的闭合指尖拓扑变更，最低通过标准是：

- 只有一只物理手可见时，渲染紧凑的 `A-B-C-D-E-A` 闭合手部面，而不是长条虚拟带；
- 两只物理手可见时，渲染包含 `AB/BC/CD/DE/EA` 条带的闭合体；
- 双手运动时能看到左手和右手端面；
- 相邻面具有不同材质/颜色处理，而不是整体同步变色；
- 手部交叉或靠近摄像头时，物体不坍缩为单张开放长板；
- 控制台没有未捕获错误，也没有 WebGL shader error。

针对部署提交 `4dd3d` 后续修复，本轮最低通过标准是：

- 只有一个物理手可见时，不再误显示 `2 hands`；
- 单手几何是折叠矩形；
- 双手几何有多个面和可见边缘；
- 首次加载没有 console error；
- 没有 `THREE.WebGLProgram: Shader Error`。
